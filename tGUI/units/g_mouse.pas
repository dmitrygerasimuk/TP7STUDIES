...
{							       Version 1.52.7 }
{							  Written by Bobby Z. }
{					   Copyright(c) 1993-95 by B-coolWare }
{ --------------------------------------------------------------------------- }
{ 
 ...
 You are hereby granted the right to use this software in either commercial
  ...
  for this code and that you keep the copyright notice unchanged. 
 ...
 If you experiencing problems with this code or have any suggestions, bug
  ...
  2:5028/52.6 (FIDOnet)
  bob@ymz.yaroslavl.su (internet) (starting in 1995)

  Internet users can also try to reach me at cnit.uniyar.ac.ru - send message
  to root@cnit.uniyar.ac.ru or xor@cnit.uniyar.ac.ru and ask to forward it
  to me.
 ...
 to g_Mouse's one). Version 1.52.7 is compatible with RTM and uses only calls
 ...
 ~~~~~~~~~~~~~~~~~~~
  ...
  5. This code written with speed in mind and uses advanced instructions of
     ...
     bother with (*$IFOPT G+*) for these CPU are quite rare now.
 ...
 ~~~~~~~~
   ...
   06 Apr 1995	 - version 1.52.7 introduced:
		 ...
		 - fixed font bank addressing bug - on VGA banks are 8K in size
		   but mapped in VRAM as #0 #4 #1 #5 #2 #6 #3 #7, thus
                   maintaining compatibility with EGA bank addressing.
...
{$ENDIF}
	...
	jmp	@@Q
	db	13,10
	db	'g_Mouse Interface  Version 1.52.7  Copyright(c) 1993-95 by B-coolWare.'
	db	13,10
@@Q:
{$IFDEF TrackVideoMode}
	pop	dx
